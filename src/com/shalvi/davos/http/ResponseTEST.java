...
public class ResponseTEST extends TestCase {
    ...
    private void assertInvalidSetCookie(String key, String val) {
        try {
            (new Response()).setCookie(key, val);
            Assert.fail();
        } catch(IllegalArgumentException e) {}
    }
    public void testSetCookie() {
        ...
        assertEquals("testval", response.getCookie("testkey"));
        assertEquals("testkey=testval;", response.getHeaderField(HeaderFieldName.SET_COOKIE).getValue());
        assertEquals(HeaderFieldName.SET_COOKIE, response.getHeaderField(HeaderFieldName.SET_COOKIE).getKey());

        assertInvalidSetCookie("", "testval");
        assertInvalidSetCookie(null, "testval");
        assertInvalidSetCookie("testkey", "");
        assertInvalidSetCookie("testkey", null);
        ...
        assertNull(response.getCookie("notavailable"));
    }
}
